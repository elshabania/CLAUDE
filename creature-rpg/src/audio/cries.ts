...
export function createCries(core: Core) {
  ...
  function voice(plan: CryPlan, t: number, vel: number) {
    ...
    if (plan.voice === 'fm' || plan.voice === 'am') {
      ...
      synth = plan.voice === 'fm'
        ? new Tone.FMSynth({ harmonicity: plan.harmonicity, modulationIndex: plan.modIndex, envelope: envOpts, modulationEnvelope: { attack: plan.attack, decay: dur * 0.6, sustain: 0.4, release: plan.release }, volume: 5 })
        : new Tone.AMSynth({ harmonicity: plan.harmonicity, envelope: envOpts, modulationEnvelope: { attack: plan.attack, decay: dur * 0.5, sustain: 0.6, release: plan.release }, volume: 6 });
      synth.connect(fxIn);
      nodes.push(synth);
