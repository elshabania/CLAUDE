...
// cumulative arc-length fraction per HORN point (the tube tapers linearly in arc length)
const ARC: number[] = [0];
for (let i = 1; i <= N; i++) ARC.push(ARC[i - 1] + Math.hypot(...HORN[i].map((v, k) => v - HORN[i - 1][k]) as V3));
const arcFrac = (t: number) => {
  const f = t * N;
  const i = Math.min(N - 1, Math.floor(f));
  return (ARC[i] + (ARC[i + 1] - ARC[i]) * (f - i)) / ARC[N];
};
const HR0 = 0.05;
const HR1 = 0.016;
const D = 180 / Math.PI;
...
function horn(side: 'L' | 'R'): PartDef[] {
  ...
  // 8 emissive ridge bands: short cylinders (axis +Y) turned onto the spiral tangent. Cylinders
  // rather than tori keep the 16 bands at ~1k triangles total.
  for (let k = 0; k < 8; k++) {
    const t = 0.12 + (k / 7) * 0.8;
    const p = m(spiralAt(t));
    const q = m(spiralAt(t + 0.01));
    const l = Math.hypot(q[0] - p[0], q[1] - p[1], q[2] - p[2]);
    const [tx, ty, tz] = [(q[0] - p[0]) / l, (q[1] - p[1]) / l, (q[2] - p[2]) / l];
    // Rx(a)·Rz(c)·(0,1,0) = (-sin c, cos c·cos a, cos c·sin a)
    const roll = -Math.asin(tx) * D;
    const pitch = Math.atan2(tz, ty) * D;
    const rr = HR0 + (HR1 - HR0) * arcFrac(t);
    parts.push({ name: `ridge${k}_${side}`, parent: `horn_${side}`, prim: { t: 'cyl', r: rr + 0.006, h: 0.014 }, at: [p[0] - tx * 0.007, p[1] - ty * 0.007, p[2] - tz * 0.007], rot: [pitch, 0, roll], slot: 'A', emissive: 1.5, mat: 'GLOW' });
  }
  return parts;
}
...
export const c05: SpeciesVisual = {
  ...
  rim: '#FFB070',
  rimStrength: 0.35,
  eye: { shape: 'almond', iris: '#FF9A2E', irisRatio: 0.72, pupil: 'h-bar', pupilRatio: 0.42, highlights: 1, lid: 0.1, lidAngle: -12 },
  mouth: { style: 'line' },
  ...
  parts: [
    ...
    { name: 'ember', parent: 'mane', mirror: true, prim: { t: 'sphere', r: 0.013 }, at: [0.09, 0.1, 0.06], slot: 'A', emissive: 1.5, mat: 'GLOW', lod: 0 },
    { name: 'ember2', parent: 'maneSide', mirror: true, prim: { t: 'sphere', r: 0.011 }, at: [0.1, 0.05, 0.05], slot: 'A', emissive: 1.5, mat: 'GLOW', lod: 0 },
    { name: 'ember3', parent: 'maneChest', mirror: true, prim: { t: 'sphere', r: 0.011 }, at: [0.06, -0.02, 0.11], slot: 'A', emissive: 1.5, mat: 'GLOW', lod: 0 },
    { name: 'ember4', parent: 'maneNape', prim: { t: 'sphere', r: 0.012 }, at: [0, 0.11, 0.03], slot: 'A', emissive: 1.5, mat: 'GLOW', lod: 0 },
    // --- neck + head ---
    { name: 'neck', parent: 'torso', prim: { t: 'capsule', r: 0.075, len: 0.2, r2: 0.065 }, at: [0, 0.08, 0.3], rot: [40, 0, 0], anim: ['look'] },
    { name: 'head', parent: 'neck', prim: { t: 'sphere', r: [0.11, 0.115, 0.15] }, at: [0, 0.3, 0.02], rot: [-30, 0, 0], scale: [1.1, 1.1, 1.1], anim: ['look'] },
    { name: 'muzzle', parent: 'head', prim: { t: 'sphere', r: [0.068, 0.065, 0.1] }, at: [0, -0.05, 0.1], rot: [10, 0, 0], slot: 'W' },
    { name: 'nostril', parent: 'muzzle', mirror: true, prim: { t: 'sphere', r: [0.012, 0.009, 0.01] }, at: [0.028, 0.012, 0.092], slot: 'D', lod: 0, anim: ['fx:nostrils'] },
    { name: 'jaw', parent: 'head', prim: { t: 'capsule', r: 0.045, len: 0.04 }, at: [0, -0.1, 0.05], rot: [95, 0, 0], slot: 'W', anim: ['jaw'] },
    { name: 'mouth', parent: 'head', prim: { t: 'mouth', r: 0.03, w: 1.5 }, at: [0, -0.095, 0.17], rot: [35, 0, 0] },
    { name: 'chinTuft', parent: 'jaw', prim: { t: 'cone', r: 0.035, h: 0.1 }, at: [0, 0.08, -0.03], rot: [150, 0, 0], slot: 'S' },
    { name: 'eye', parent: 'head', mirror: true, prim: { t: 'eye', r: 0.056 }, at: [0.076, 0.035, 0.085], rot: [0, 38, 0], scale: [1.2, 1.05, 1] },
