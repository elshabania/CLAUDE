...
## Addendum (Creature Art Director, 2026-09-25)

**D31 — Flagship redesigns + creature rendering upgrade** (evidence: design/reviews/creature_art_qa.md "v2 pass"). (1) **Rendering (all 30 species):** organic parts are sculpted as one signed-distance field per material group with smooth blending between each part and its parent, meshed with surface nets, cached per species/LOD/quality (idle pre-warm via `prewarmCreatures`), and skinned to the part nodes so the procedural animator still bends bodies. Hard accessories stay crisp meshes. New per-class procedural surfaces (fur, scales, feathers, skin, stone, shell, ice, membrane), wrap/subsurface lighting, molten-fissure glow masks and High-tier shell fur. Eyes are real geometry (eyeball, painted iris, clearcoat cornea, eyelids); the 8 D14 face states are lid poses. Budgets: whole creature ≈ 25k tris at High LOD0 (sculpted body ≤ 12.5k), ≈ 16k Balanced, ≈ 8.5k Mobile. The f09 cut-out line (c25–c27) keeps its flat plates and atlas eyes by design. (2) **f01 line:** c01 Fizzkit becomes the mascot-level electric critter (silver-lilac dormouse × sugar glider, satin ears with static rims, blue capacitor freckles, wrist-to-ankle membranes, coiled bulb tail); c02 glider-sprinter; c03 storm glider. Names kept. (3) **f02 line:** ram-horned fire dragon line (wick-flame horns kept); c06 renamed Magmouflon → **Smoulderam** and retyped **fire · gale**; its evolution move becomes m069 Gale Cleaver (was m048 Quake Stomp; same power and accuracy). Stats totals are unchanged. creatures.md is now v3; species.json is regenerated by `scripts/build-species.py`. (4) **Balance:** `balance.test.ts` and the campaign/world tests still pass with every blocking target met. Fire-starter Cantor 5 fell from 100% to 73% (target ≥ 60). Its champion win rate rose from 63% to 73%.
