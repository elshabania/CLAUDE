...
from ..models import Finding, KPI, Severity
...
Block = Union[Heading, Paragraph, BulletList, Table, Figure, PageBreak, Callout]
...
GLOSSARY: list[tuple[str, str]] = [
    ("V/C", "Volume-to-capacity ratio: link volume per hour divided by link capacity per hour. "
            "Values above 1.0 mean demand exceeds capacity."),
    ("GEH", "A statistic comparing two flows, M and C: sqrt(2 (M - C)^2 / (M + C)). "
            "Values below 5 are usually treated as a good match."),
    ("Relative gap", "Assignment convergence measure: the excess cost of the current route "
                     "choices over the cheapest possible routes, divided by total cost. "
                     "Smaller is better."),
    ("Noise band", "The size of link flow change between the last assignment iterations. "
                   "Differences inside the band are not treated as significant."),
    ("Centroid connector", "An artificial link that loads a zone's trips onto the network at "
                           "a chosen node."),
    ("Screenline", "An imaginary line across the network on which modelled and observed "
...
def _cover(meta: ReportMeta) -> list[Block]:
    ...
    blocks.append(Table(["Item", "Value"], rows, caption="Run details",
                        source_ref=_src("manifest.json")))
...
def _executive_summary(store: RunStore, ranked: list[Finding]) -> list[Block]:
    ...
    if n_total == 0:
        blocks.append(Paragraph("The checks produced no findings for this run.",
    ...
    if not top:
        ...
        rows = [[str(i + 1), f.severity.value, _location_text(f), f.executive_line]
                for i, f in enumerate(top)]
        blocks.append(Table(["Rank", "Severity", "Location", "Finding"], rows,
...
def _fix_first(ranked: list[Finding], limit: int = 10) -> list[Block]:
    ...
    action = [f for f in ranked
              if f.severity in (Severity.CRITICAL, Severity.HIGH) and f.is_significant]
    if not action:
        blocks.append(Paragraph(
    ...
    blocks.append(Paragraph(
        ...
        source_ref=_src("findings.json")))
    rows = [[f.severity.value, _location_text(f), f.executive_line, f.suggested_action,
             _best_measure(f)] for f in shown]
    blocks.append(Table(
        ["Severity", "Location", "Finding", "Suggested action", "Best measure"], rows,
        caption="Action list",
    ...
    if len(action) > limit:
        blocks.append(Paragraph(
            ...
            f"findings section.", source_ref=_src("findings.json")))
    return blocks
...
def _finding_blocks(f: Finding) -> list[Block]:
    ...
    if not f.is_significant:
        blocks.append(Paragraph("This finding is inside the noise band and is not significant.",
                                source_ref=src))
    ...
    if ev.values or ev.thresholds:
        ...
        rows = []
        for k in keys:
            rows.append([k, fmt_evidence(k, ev.values.get(k)) if k in ev.values else "",
                         fmt_evidence(k, ev.thresholds.get(k)) if k in ev.thresholds else ""])
        blocks.append(Table(["Metric", "Value", "Threshold"], rows,
                            caption="Evidence", source_ref=src))
    if ev.sources:
    ...
    if f.measures:
        rows = [[m.title, m.description, m.estimated_effect or "Not estimated",
                 m.method.value.replace("_", " "), m.confidence] for m in f.measures]
        blocks.append(Table(["Measure", "Description", "Estimated effect", "Method",
                             "Confidence"], rows, caption="Proposed measures", source_ref=src))
    return blocks
    ...
    for s in SEVERITY_ORDER:
        ...
        if not group:
            blocks.append(Paragraph(
                ...
                f"see derived/findings.json.", source_ref=_src("findings.json")))
            continue
        for f in group:
...
def _convergence(store: RunStore, ranked: list[Finding],
                 figures: Mapping[str, tuple[Path, str]]) -> list[Block]:
    ...
    if res is None:
        ...
        blocks.append(Paragraph(
            ...
            f"message: {msg}.", source_ref=_src("check_results.json")))
    ...
    if conv_findings:
        ...
        blocks.append(Table(["Severity", "Location", "Finding"], rows,
                            caption="Convergence findings",
                            source_ref=_src(*[f"findings/{f.finding_id}"
                                              for f in conv_findings])))
    else:
        blocks.append(Paragraph("The convergence check raised no findings.",
                                source_ref=_src("findings.json")))
    ...
    if store.has("convergence"):
        ...
        params = config.checks().get("checks", {}).get("convergence_and_noise", {}).get(
            "params", {})
        ...
        for r in df.itertuples():
            ...
            rows.append([str(r.stage), str(r.period), str(r.metric), str(int(r.iterations)),
                         fmt(float(r.final_value)), met])
        blocks.append(Table(
            ["Stage", "Period", "Metric", "Iterations", "Final value", "Target met"], rows,
            caption=(f"Final convergence values (relative gap target {fmt(target)})"
                     if target is not None else "Final convergence values"),
    ...
    if fig is not None:
        blocks.append(Figure(fig[0], "Relative gap by iteration and period (log scale)",
                             source_ref=fig[1]))
    return blocks
...
def _kpis(store: RunStore) -> list[Block]:
    ...
    blocks.append(Table(["KPI", "Value", "Period", "Definition"], rows,
...
def _check_coverage(store: RunStore) -> list[Block]:
    ...
    rows = [[str(r.get("check_name") or r.get("check_id")), str(r.get("status")),
             str(int(r.get("n_findings", 0))), str(r.get("message") or ""),
             f"{float(r.get('duration_s', 0.0)):.2f}"] for r in results]
    blocks.append(Table(["Check", "Status", "Findings", "Message", "Duration (s)"], rows,
                        caption="Check coverage", source_ref=_src("check_results.json")))
    return blocks
...
def _appendix() -> list[Block]:
    ...
    for cid, spec in checks.items():
        ...
        ptext = "; ".join(f"{k} = {fmt(v) if not isinstance(v, dict) else v}"
    ...
    blocks.append(Table(["Check", "Enabled", "Parameters", "Severity rules"], rows,
                        caption="Check thresholds and severity rules",
                        source_ref=_src("config/checks.yaml")))
    blocks.append(Heading(2, "Glossary"))
    blocks.append(Table(["Term", "Meaning"], [[t, d] for t, d in GLOSSARY], caption="Glossary",
...
def build_diagnostic_report(
    ...
    meta = ReportMeta(
        ...
        if manifest.is_synthetic else None,
    )
