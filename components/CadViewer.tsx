...
export function CadViewer({
  ...
  useEffect(() => {
    ...
    // VISSIM-style network overlay: one-way Links (single centerline,
    // thickness ∝ numLanes), and Connectors at every junction (Bézier curves
    // between Link ends, colored by turn). Junctions are implicit — a cluster
    // of Connectors *is* a junction.
    if (showLanes && laneNetwork && laneNetwork.links.length > 0) {
      ctx.save();
      ctx.lineCap = "round";
      ctx.lineJoin = "round";

      // Connectors first (under the links): coloured curves joining link ends.
      if (scale > 0.02 && laneNetwork.connectors.length > 0) {
        ...
        for (const con of laneNetwork.connectors) {
          ...
          for (let i = 0; i < cp.length; i += 2) {
            const s = toScreenPt(cp[i], cp[i + 1]);
      ...
      // Links — single centerline per link, stroke width ∝ numLanes.
      for (const link of laneNetwork.links) {
        const pts = link.points;
        if (pts.length < 4) continue;
        ctx.strokeStyle = laneCountColor(link.numLanes);
        ctx.lineWidth = Math.max(1.5, 1 + link.numLanes * 1.6);
        ctx.globalAlpha = link.directed ? 1 : 0.6;
        ctx.beginPath();
        ...
        if (scale > 0.04) {
          ...
          ctx.fillStyle = laneCountColor(link.numLanes);
          ctx.save();
        ...
        // numLanes label (shows the VISSIM-style Link attribute when zoomed in)
        if (scale > 0.08) {
          const mi = (pts.length >> 2) * 2;
          const a = toScreenPt(pts[mi], pts[mi + 1]);
